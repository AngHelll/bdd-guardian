...
BDD Guardian exposes a **read-only Index API v1** via `extension.exports` for companion extensions (e.g. BDD Pilot pre-run gates and other in-process consumers).
...
Published Marketplace companions today: Guardian + Pilot. Other tools may consume this Index API privately; they are not listed as shipped suite products until public release.

Workspaces with features but no step definitions (e.g. Gherkin-only samples) may show `bindings: 0` even when `source: guardian` — that is expected.
...
3. Poll `isReady` up to ~10s before falling back to a local/basic mode
